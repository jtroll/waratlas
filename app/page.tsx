...
import { getActiveConflicts, getAutoSpeed, conflictCitation } from '@/lib/conflicts';
...
// scripts/data/id_redirects.json by scripts/generate-stats.mjs).
...
export default function Home() {
    ...
    fetch('/conflicts.json')
      ...
      .then((data: unknown) => {
        ...
        if (!Array.isArray(data) || data.length === 0) {
          throw new Error('conflicts.json is not a non-empty array');
        }
      ...
      .catch((err: unknown) => {
        ...
        console.error('War Atlas: failed to load conflicts.json', err);
  ...
  useEffect(() => {
    fetch('/empires.json')
      ...
      .then((d) => {
        ...
        setEmpireIndex(
          features.map((f) => {
            const p = (f.properties ?? {}) as { name?: unknown; startYear?: unknown; endYear?: unknown };
            return {
              name: String(p.name ?? ''),
  ...
  // Latest conflicts for the playback loop — the effect is keyed on
  // isPlaying only, so it reads the array through a ref instead of a stale
  // closure.
    ...
    const tick = (now: number) => {
      ...
      setTimeline(prev => {
        ...
        const speed = prev.speedMode === 'auto'
          ? getAutoSpeed(prev.currentYear, conflictsRef.current)
          : prev.playbackSpeed;
  ...
  return (
    ...
    <main className="relative w-screen h-screen overflow-hidden bg-wars-bg" style={{ height: '100dvh' }}>
      ...
      {loadError ? (
        <DataLoadError message={loadError} onRetry={() => setLoadAttempt((a) => a + 1)} />
      ) : !mapReady && (
